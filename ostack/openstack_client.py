...
def list_servers(conn,name_filter):
    servers_list=[]
    for server in conn.compute.servers():
        if(server.name[0:12] == name_filter):
...
def find_server(conn, name_server):
    res = conn.compute.find_server(name_server)
    return res

#find_server(authentication(),'worker-node-0erl')
...
#create new flavor 
    ...
    f = open('/opt/stack/Code/ostack/user_data.txt','r')
    user_data = f.read()
...
def list_all_server_ids(conn):
    server_ids_list=[]
    for server in conn.compute.servers():
        server_ids_list.append(server.id)
    return server_ids_list

def get_sum_cpu_and_memory_servers(conn):
    server_ids_list = list_all_server_ids(conn)
    resource = dict()
    cpu = 0
    memory = 0
    for server_id in server_ids_list:
        flavor = conn.compute.get_server(server_id).flavor
        cpu += flavor['vcpus']
        memory += flavor['ram']
    resource['cpu'] = cpu
...
#print(get_sum_cpu_and_memory_servers(authentication()))
